...
export default function Meet() {
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 p-4 flex flex-col">
          <div className="flex-1 grid grid-cols-2 gap-4 mb-4">
            {/* Candidate Video */}
            <div className="relative bg-[#45767C] rounded-xl overflow-hidden group">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-32 h-32 bg-[#9CBFAC] rounded-full flex items-center justify-center">
                  <span className="text-[#29445D] text-4xl font-bold">SJ</span>
                </div>
              </div>
              <div className="absolute bottom-4 left-4 bg-[#D1DED3] bg-opacity-60 px-3 py-1.5 rounded-lg backdrop-blur-sm">
                <span className="text-[#29445D] text-sm font-medium">Sarah Johnson (Candidate)</span>
              </div>
            ...
            <div className="relative bg-[#45767C] rounded-xl overflow-hidden group">
              <div className="absolute inset-0 flex items-center justify-center">
                {isVideoOff ? (
                  ...
                  <div className="w-32 h-32 bg-[#9CBFAC] rounded-full flex items-center justify-center">
                    <span className="text-[#29445D] text-4xl font-bold">HR</span>
                  </div>
                )}
              </div>
              <div className="absolute bottom-4 left-4 bg-[#D1DED3] bg-opacity-60 px-3 py-1.5 rounded-lg backdrop-blur-sm">
                <span className="text-[#29445D] text-sm font-medium">You (Interviewer)</span>
              </div>
              <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition">
                <button className="p-2 bg-black bg-opacity-60 rounded-lg backdrop-blur-sm text-white hover:bg-opacity-80">
                  <Maximize className="w-4 h-4" />
              ...
              {isMuted && (
                <div className="absolute top-4 left-4">
                  <div className="p-2 bg-[#29445D] rounded-lg">
          ...
          {/* Controls Bar */}
          <div className="bg-[#45767C] rounded-xl p-4 flex items-center justify-between">
  {/* Left Controls */}
  <div className="flex items-center space-x-2">
    <button
      ...
      className={`p-4 rounded-xl transition ${
        isMuted
          ? 'bg-[#29445D] hover:bg-[#719D99]'
          : 'bg-[#D1DED3] hover:bg-[#9CBFAC]'
      }`}
    >
      {isMuted ? (
        <MicOff className="w-5 h-5 text-[#D1DED3]" />
      ) : (
        <Mic className="w-5 h-5 text-[#29445D]" />
      )}
    </button>
    ...
    >
      {isVideoOff ? (
        <VideoOff className="w-5 h-5 text-[#D1DED3]" />
      ) : (
        <Video className="w-5 h-5 text-[#29445D]" />
      )}
    </button>
  </div>
  ...
  <div className="flex items-center space-x-2">
    <button
      ...
      className={`p-4 rounded-xl transition ${
        showChat ? 'bg-[#719D99]' : 'bg-[#D1DED3] hover:bg-[#9CBFAC]'
      }`}
    >
      <MessageSquare className="w-5 h-5 text-[#29445D]" />
    </button>
    ...
    <button
      ...
      className={`p-4 rounded-xl transition ${
        showParticipants ? 'bg-[#719D99]' : 'bg-[#D1DED3] hover:bg-[#9CBFAC]'
      }`}
    >
      <Users className="w-5 h-5 text-[#29445D]" />
    </button>

    <button className="p-4 rounded-xl bg-[#D1DED3] hover:bg-[#9CBFAC] transition">
      <MoreVertical className="w-5 h-5 text-[#29445D]" />
    </button>
  </div>

  {/* End Button */}
  <button className="px-6 py-4 bg-[#29445D] hover:bg-[#719D99] rounded-xl transition flex items-center space-x-2">
